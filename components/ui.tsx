    ...
    <div className="min-h-screen bg-background lg:grid lg:grid-cols-[15.5rem_minmax(0,1fr)]">
      ...
      <aside className="app-sidebar hidden min-h-screen flex-col bg-[hsl(var(--header))] text-[hsl(var(--header-foreground))] lg:sticky lg:top-0 lg:flex lg:h-screen">
        ...
        <div className="mx-4 mb-5 border-t border-white/15 pt-4">
          ...
          <p className="mt-4 px-3 font-mono text-[0.68rem] text-white/40">FortiGate Backup Portal</p>
        </div>
      </aside>
      <div className="min-w-0">
        <header className="app-header sticky top-0 z-30 border-b border-border bg-surface/95 backdrop-blur-sm">
          <div className="flex min-h-[4.5rem] items-center gap-3 px-4 sm:px-6 lg:px-8">
            ...
            {user ? <details className="group lg:hidden"><summary className="app-nav-summary topbar-icon cursor-pointer"><span aria-hidden="true" className="relative block h-4 w-5"><span className="absolute left-0 top-0 h-0.5 w-5 rounded bg-current transition-transform group-open:translate-y-[7px] group-open:rotate-45"/><span className="absolute left-0 top-[7px] h-0.5 w-5 rounded bg-current transition-opacity group-open:opacity-0"/><span className="absolute bottom-0 left-0 h-0.5 w-5 rounded bg-current transition-transform group-open:-translate-y-[7px] group-open:-rotate-45"/></span><span className="sr-only">Menu</span></summary><nav aria-label="Mobiele hoofdnavigatie" className="absolute inset-x-3 top-[4.25rem] grid gap-1 rounded-xl border border-border bg-surface p-2 shadow-xl"><div className="mb-1 border-b border-border p-2"><TenantSwitcher key={user.activeTenantId ?? "mobile-no-tenant"} action={switchTenantContextAction} activeTenantId={user.activeTenantId} canSwitch={canSwitchTenants} tenantName={tenantName} tenants={tenants}/></div>{navigation}</nav></details> : null}
          </div>
        </header>
...
function NavigationLinks({ isBreakGlassSettingsOnly, isGlobalContext, canManageTenants, canReadUsers, canReadAudit }: { isBreakGlassSettingsOnly: boolean; isGlobalContext: boolean; canManageTenants: boolean; canReadUsers: boolean; canReadAudit: boolean }) {
  ...
  return <>
    ...
    {!isGlobalContext ? <><AppNavLink href="/customers"><Icon name="device"/>Klanten & FortiGates</AppNavLink><AppNavLink href="/alerts"><Icon name="alert"/>Alerts</AppNavLink></> : null}
