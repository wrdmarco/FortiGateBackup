  ...
  return (
    <main className="grid min-h-screen bg-background lg:grid-cols-[minmax(0,1.1fr)_minmax(28rem,.9fr)]">
      <section className="relative hidden overflow-hidden bg-[hsl(var(--header))] p-12 text-[hsl(var(--header-foreground))] lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative flex items-center gap-3"><span className="brand-sigil" aria-hidden="true"><span /></span><span><span className="block text-sm font-bold tracking-[0.28em]">FORTI</span><span className="block text-sm font-bold tracking-[0.2em]">BACKUP</span></span></div>
        <div className="relative max-w-xl"><p className="mb-4 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-200">Security operations</p><h2 className="font-display text-5xl font-semibold leading-[1.08] tracking-[-0.04em]">Elke configuratie.<br/>Veilig vastgelegd.</h2><p className="mt-6 max-w-lg text-lg leading-7 text-white/65">Centraal beheer, geverifieerde snapshots en een volledig auditspoor voor iedere FortiGate binnen je MSP-omgeving.</p></div>
      ...
      <section className="grid place-items-center px-4 py-10 sm:px-8">
        <div className="w-full max-w-[430px]">
          <div className="mb-10 flex items-center gap-3 lg:hidden"><span className="brand-sigil brand-sigil-small" aria-hidden="true"><span /></span><span className="text-xs font-bold tracking-[0.18em]">FORTI BACKUP</span></div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-success">Welkom terug</p>
          <h1 className="mt-3 font-display text-4xl font-semibold tracking-[-0.04em]">Inloggen</h1>
