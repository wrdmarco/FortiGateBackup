...
export async function retryAnalysisAction(formData:FormData){const user=await requirePermission("security.analyses.run");const tenantId=tenantFilter(user);const analysisId=String(formData.get("analysisId")??"");if(!tenantId)throw new Error("Tenantcontext ontbreekt.");await tenantTransaction(tenantId,async(tx)=>{const analysis=await tx.securityAnalysis.findFirstOrThrow({where:{id:analysisId,tenantId}});if(analysis.status==="COMPLETED")return;await tx.securityAnalysis.update({where:{id:analysis.id},data:{status:"PENDING",errorCode:null}});const job=await tx.securityAnalysisJob.update({where:{analysisId:analysis.id},data:{status:"PENDING",availableAt:new Date(),attempts:0,errorCode:null,finishedAt:null,workerId:null,leaseExpiresAt:null}});await tx.securityAnalysisJobEvent.create({data:{tenantId,jobId:job.id,stage:"MANUAL_RETRY",message:"Een bevoegde gebruiker heeft de analyse opnieuw ingepland."}});});await auditLog({action:"security.analysis.retry_requested",tenantId,userId:user.id,entity:"SecurityAnalysis",entityId:analysisId});revalidatePath("/security");revalidatePath("/queue");}
