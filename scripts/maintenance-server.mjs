...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body{min-height:100%;margin:0}body{background:var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;letter-spacing:0}
    .topbar{height:64px;background:var(--surface);border-bottom:1px solid var(--line);display:flex;align-items:center;padding:0 24px}.brand{display:flex;align-items:center;gap:12px;font-size:15px;font-weight:750}.mark{display:grid;place-items:center;width:34px;height:34px;border-radius:6px;background:var(--accent);color:#fff;font-size:12px;font-weight:850}
    ...
    .status{display:grid;grid-template-columns:minmax(0,1fr) 290px;gap:24px;margin-top:36px;padding-top:28px;border-top:1px solid var(--line)}.progress{height:8px;margin:22px 0 14px;overflow:hidden;border-radius:4px;background:var(--line)}.progress span{display:block;width:38%;height:100%;background:var(--accent);animation:move 1.4s ease-in-out infinite}.meta{display:flex;flex-wrap:wrap;gap:10px 20px;color:var(--muted);font-size:13px}.summary{align-self:start;border-left:3px solid var(--accent);padding:2px 0 2px 18px}.summary strong{display:block;font-size:15px}.summary span{display:block;margin-top:8px;color:var(--muted);font-size:13px;line-height:1.55}
    ...
    @media(max-width:720px){.topbar{height:58px;padding:0 16px}main{width:min(100% - 32px,1080px);padding-top:36px}.status{grid-template-columns:1fr}.summary{border-left:0;border-top:3px solid var(--accent);padding:16px 0 0}.log pre{max-height:38vh}}
    @media(prefers-reduced-motion:reduce){.progress span{animation:none;width:70%}}
  </style>
</head>
<body>
  <header class="topbar"><div class="brand"><span class="mark" aria-hidden="true">FG</span><span>FortiGate Backup Portal</span></div></header>
  <main id="main">
  ...
  <script>
    (()=>{
      ...
      const apply=s=>{
        ...
        if(s.log!==null){logPanel.classList.add("visible");log.textContent=s.log||"Wachten op update-output...";log.scrollTop=log.scrollHeight}else{logPanel.classList.remove("visible")}
